...
WITH
	...
	SCORED_EVENTS AS (
		SELECT
			...
			END AS ROLE_WEIGHT,
			EXP(
				-0.693 * EXTRACT(EPOCH FROM (NOW() - DE.CREATED_AT))
				/ (14 * 86400)
			) AS RECENCY_WEIGHT,
			W.TYPE_WEIGHT
		FROM
	...
	NORMALIZED AS (
		SELECT
			...
			LEAST(
				ROUND(
					(LN(1 + CR.RAW_SCORE) / NULLIF(LN(1 + C.MAX_SCORE), 0)) * 60
				) + LEAST(
					ROUND((CR.ATTACKS_LAUNCHED + CR.ATTACKS_RECEIVED) / 10.0 * 40),
					40
				),
				100
			) AS THREAT_SCORE
...
SELECT * FROM V_COUNTRY_THREAT_SCORE;