DROP VIEW IF EXISTS V_COUNTRY_THREAT_SCORE;

CREATE VIEW V_COUNTRY_THREAT_SCORE AS
WITH
	WEIGHTS AS (
		SELECT
			'MIL' AS CONFLICT_TYPOLOGY,
			1.0 AS TYPE_WEIGHT
		UNION ALL
		SELECT
			'MOVE',
	...
	COUNTRY_RAW AS (
		SELECT
			COUNTRY,
			SUM(
				IMPORTANCE_SCORE * RECENCY_WEIGHT * TYPE_WEIGHT * ROLE_WEIGHT
			) AS RAW_SCORE,
			COUNT(DISTINCT TWEET_ID) AS EVENT_COUNT,
			MAX(IMPORTANCE_SCORE) AS MAX_SEVERITY,
			COUNT(
				DISTINCT CASE
					WHEN ROLE_RANK = 3 THEN TWEET_ID
				END
			) AS ATTACKS_LAUNCHED,
			COUNT(
				DISTINCT CASE
					WHEN ROLE_RANK = 2 THEN TWEET_ID
				END
			) AS ATTACKS_RECEIVED
		FROM
			SCORED_EVENTS
	...
	NORMALIZED AS (
		SELECT
			...
			LEAST(
				ROUND(
					(
						LN(1 + CR.RAW_SCORE) / NULLIF(LN(1 + C.MAX_SCORE), 0)
					) * 60
				) + LEAST(
					ROUND(
						(CR.ATTACKS_LAUNCHED + CR.ATTACKS_RECEIVED) / 10.0 * 40
					),
					40
				),
				100
...
SELECT
	...
	MAX_SEVERITY,
	CASE
		WHEN THREAT_SCORE >= 80 THEN 'Open warfare'
		WHEN THREAT_SCORE >= 60 THEN 'Active conflict'
		WHEN THREAT_SCORE >= 40 THEN 'High threat'
		WHEN THREAT_SCORE >= 20 THEN 'Moderate threat'
		ELSE 'Calm'
	END AS THREAT_LEVEL
FROM
	NORMALIZED
